...
import { countandfetchCaseRoute,Addcourt } from '../../utlis/APIRoutes';
...
function Assigncourt() {
  ...
  const   handleAccept = async(caseid)=>{
    const Approve = Judgename.court_details
    console.log("court details " ,Approve)
    try {
     const response = await axios.put(Addcourt, {
        caseId: caseid,
        courtdetails: Approve
      });
      setalert(true)
...
{alertSuccess &&  <AlertSuccess msg="Court Details is assigned Succesfully to this Case" /> }
      ...
      <div className="modal-content">
        ...
        <div className="modal-body">
          <div className="card" style={{ width: '28rem' }}>
            ...
            <div className="mb-3">
            <label htmlFor="exampleInputjudge1" className="form-label mx-2 mt-2 ml-3 mb-2">Enter Court Details </label>
            <textarea type="text" rows={4} className="form-control w-75 ml-3 mr-3 mt-2 mb-2" name='court_details' id="exampleInputEmail1" onChange={(e) => handleChange(e)} aria-describedby="emailHelp" placeholder='Court Details' />
            </div>
            </ul>
          </div>
        </div>
        <div className="modal-footer">
          ...
          <button type="button" className="btn btn-info" data-bs-dismiss="modal" onClick={()=>{handleAccept(caseItem._id)}}>Assign a Court</button>
        </div>
      </div>
    </div>
  </div>
